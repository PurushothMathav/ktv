...
const CACHE_VERSION = 'v0908251640';
const CACHE_NAMES = {
  STATIC: `ktv-static-${CACHE_VERSION}`,
  DYNAMIC: `ktv-dynamic-${CACHE_VERSION}`,
  IMAGES: `ktv-images-${CACHE_VERSION}`,
  JSON: `ktv-json-${CACHE_VERSION}`,
  FONTS: `ktv-fonts-${CACHE_VERSION}`
};
...
// Cache size limits
const CACHE_LIMITS = {
  IMAGES: 2000, // Maximum 100 images
  JSON: 2000,   // Maximum 200 JSON files
  DYNAMIC: 2000  // Maximum 50 dynamic resources
};
    ...
    Promise.all([
      ...
      caches.keys().then((cacheNames) => {
        const deletePromises = cacheNames.map((cacheName) => {
          if (cacheName.startsWith('ktv-') && !Object.values(CACHE_NAMES).includes(cacheName)) {
...
async function handleForceUpdate() {
  try {
    ...
    await Promise.all(
      cacheNames
        .filter(name => name.startsWith('ktv-'))
